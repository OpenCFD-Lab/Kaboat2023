...
while runOption:
    try:
        ...
        if updateGPS:
            ...
            SBLeeData = str(currLT) + ' ' + str(currRT) + ' ' + str(currCT) + ' ' + str(velocity) + ' ' + str(betweenDistance) + ' ' + str(diffAngle) + ' ' + str(deltaTime) + '\n'
            #with open('./jbAnData/SBLEE_1.dat', 'a') as SBLeeFile:
            #    SBLeeFile.write(SBLeeData)
            print('LT : ', currLT, ' | RT : ', currRT, ' | CT : ', currCT, ' | V : ', round(velocity,5))
            print('D : ', round(betweenDistance,5), ' | angle : ', round(diffAngle,5), ' | dT : ', round(deltaTime,5))
            print('########################################################')
            '''
