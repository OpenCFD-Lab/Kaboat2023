...
if criticalError:
    while True:
        print('CRITICAL ERROR : ARDUINO PORT IS NOT DEFINE')
        pi_connect.set_servo_pulsewidth(19,1500)
        pi_connect.set_servo_pulsewidth(20,1500)
        pi_connect.set_servo_pulsewidth(5,1500)
...
while RCmode == False:
    try:
        with open('/home/pi/OpenDEP/Receive/Thruster/thruster.dat','r') as thrusterFile:
            PWMData = thrusterFile.readline().strip('\n').split(' ')
            LT = int(PWMData[0])
            RT = int(PWMData[1])
            CT = int(PWMData[2])
    except:
