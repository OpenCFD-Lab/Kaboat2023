...
int fNi=0;
string rawPath="/home/pi/lidarWorkSpace/raw.dat";
...
int main(int argc, const char * argv[]) {
    ...
    while (1) {
        string fNS=std::to_string(fNi);
        string filePath="/home/pi/lidarWorkSpace/disk/Lidar_"+fNS+".dat"; 
        //string filePath="/home/pi/lidarWorkSpace/disk/Lidar.dat"; 
        ofstream path(filePath);
        ...
        if (SL_IS_OK(op_result)) {
            ...
            {
                double rAngle=(nodes[pos].angle_z_q14*90.f)/16384.f;
                double rDist=nodes[pos].dist_mm_q2/4.0f/1000;
                if (fNi==0) rawFile << rAngle << " " << rDist << endl;
                if (rAngle>targetAngle && rAngle<targetAngle+1) 
                {
                    if(rDist)
                    {
                        sum+=rDist;
                ...
                else if (rAngle>targetAngle+1)
                {
                    if (sum>0) avg=sum/n;
                    else avg=0; 
                    path << avg << " ";
                    if (targetAngle<6) std::cout << targetAngle << " deg, " << avg << " m" << std::endl;
                    else if (targetAngle>354) std::cout << targetAngle-360 << " deg, " << avg << " m" << std::endl;
                    if(rDist>0)
                    {
                        sum=rDist;
                        n=1;
            ...
            if (fNi==0) rawFile.close();
            if (sum>0)avg=sum/n;
            else avg=0;
            path << avg;
            std::cout << targetAngle-360 << " deg, " << avg << " m" << std::endl;
            std::cout << "-------------------------------" << std::endl;
            fNi++;
        }
        path.close();
